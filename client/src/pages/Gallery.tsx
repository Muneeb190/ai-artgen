...
const Gallery = () => {
  ...
  const fetchPosts = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/api/post?sort=${sortBy}`,
  ...
  const handleLike = async (postId: string) => {
    ...
    try {
      if (!isLiked) {
        await fetch(`http://localhost:5000/api/post/${postId}/like`, { method: "PATCH" });
      }
      setLikedPosts((prev) =>
        isLiked
  ...
  const handleView = async (postId: string) => {
    try {
      await fetch(`http://localhost:5000/api/post/${postId}/view`, { method: "PATCH" });
      setAllPosts((prev) =>
        prev.map((post) =>
